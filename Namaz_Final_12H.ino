...
void setup(){
  ...
  lcd.init();
  lcd.backlight();
  if (!rtc.begin()) { lcd.print("RTC Error"); while(1); }

  DateTime compiled = DateTime(F(__DATE__), F(__TIME__));
  if (rtc.now() < compiled) rtc.adjust(compiled);
...
void displayManager(DateTime dt, long jDate) {
  ...
  if (strlen(activePrayer) > 0) {
    ...
    // --- HEADER (No AM/PM, just clear time) ---
    ...
    if (page == 0) { // Morning
      ...
      padTo16(r1); padTo16(r2); padTo16(r3);
    }
  }
