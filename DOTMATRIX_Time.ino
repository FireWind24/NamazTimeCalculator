...
void setup() {
  ...
  // Now listen for exactly 12 digit bytes
  ...
  while (millis() - waitStart < 15000) {
    if (Serial.available() >= 12) {
      ...
      if (valid) {
        ...
        // Got garbage — flush and keep waiting
        while (Serial.available()) Serial.read();
      }
    }
  }
...
void loop() {
  ...
  scrollPage = (scrollPage + 1) % 5;
}

// =============================================
// MESSAGE BUILDER
// =============================================
void buildMessage(DateTime now) {
  ...
  switch (scrollPage) {
    case 0:
      sprintf(scrollBuf, "%02d:%02d:%02d", h12, now.minute(), now.second());
      break;
    case 1:
